...
## Data Types
All [data types](Data_Type) are Cartesian Products of two integers: id and value.
```
...
## Channels
Once we have the [data types](Data_Type), [channels](ChanDefs) needed for modelling this flow can be defined as:
```
...
### sgn
The sign [function](FuncDefs): zero for zero, +1 for all positive numbers, and -1 for all negative numbers.
```
...
![Correction process](https://github.com/TorXakis/TorXakis/blob/develop/examps/controlloop/correct.jpg)
...
![Measuring process](https://github.com/TorXakis/TorXakis/blob/develop/examps/controlloop/measure.jpg)
...
![Production process](https://github.com/TorXakis/TorXakis/blob/develop/examps/controlloop/produce.jpg)
...
![Control loop](https://github.com/TorXakis/TorXakis/blob/develop/examps/controlloop/controlloop.jpg)
...
Since the above Control Loop process (model) puts everything together, now we can [define our model](ModelDefs) as follows:
```
MODELDEF Model ::=
    CHAN IN
...
1.  Start TorXakis: run [TorXakis](TorXakis) with the Control Loop model describe above in a command window.
